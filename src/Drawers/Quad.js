...
class AveragedQuad {
  ...
  draw() {
    const color = this.colors[8];
    this.p.fill(this.p.color(color));

    this.p.strokeWeight(4);

    // this.p.quad(this.x, this.y, this.x + 100, this.y, this.x + 20, this.y - 40, this.x + 10, this.y - 50);
    this.p.beginShape();
    this.p.vertex(this.x - 20, this.y + 10);
    this.p.vertex(this.x, this.y + 35);
    this.p.vertex(this.x + 1, this.y + 10);
    this.p.vertex(this.x + 70, this.y + 0);
    this.p.vertex(this.x + 1, this.y + -8);
    this.p.vertex(this.x, this.y + -35);
    this.p.vertex(this.x - 20, this.y + -8);
    this.p.vertex(this.x - 35, this.y);
    this.p.endShape();
  }
...
export class AveragedQuads {
  ...
  addQuad() {
    ...
    while (numAttempts < this.MAX_ATTEMPTS) {
      ...
      if (isValid) {
        this.squares.push(new AveragedQuad(this.p, x, y, d, isSource));
        numAdded++;
      }
      if (numAdded === target) {
        break;
